(() => {
  ...
  function drawDashEffect() {
    ...
    const alpha = 1 - p;
    ctx.save();
    ctx.translate(s.x, s.y - 12); ctx.rotate(dashFx.angle);
    ctx.globalAlpha = alpha * .75; ctx.strokeStyle = "rgba(205,235,255,.95)"; ctx.lineWidth = 4;
    ctx.beginPath(); ctx.ellipse(0, 0, 24 + p * 18, 10 + p * 7, 0, 0, Math.PI * 2); ctx.stroke();
    ctx.globalAlpha = alpha * .38;
    ctx.beginPath(); ctx.moveTo(-20, 0); ctx.lineTo(-55, 0); ctx.moveTo(-17, 7); ctx.lineTo(-46, 12); ctx.stroke();
    ctx.restore();
  }
  ...
  window.updatePlayer = function(dt, now) {
    ...
    if (moved > worldPixels(35) && !player.attack) {
      dashFx = { x: player.x, y: player.y, start: now, angle: Math.atan2(player.y - beforeY, player.x - beforeX) };
    }
  };
  ...
  function ensureWave4Bosses() {
    ...
    if (!summoner) {
      ...
      showMessage("ONDA 4 — CHEFÃO INVOCADOR APARECEU!");
    }
  }
  ...
  window.updateBossDangerZones = function(now) {
    if (typeof enemies === "undefined") return;
    const filtered = enemies.filter(e => e.type !== "summonerBoss");
    try {
  ...
  function updateLightning(now) {
    ...
    for (const enemy of enemies) {
      ...
      if (now >= enemy.lightningNextTick) {
        ...
        if (enemy.hp <= 0) {
          enemy.state = "dead"; enemy.attackPhase = null; rewardEnemy(enemy, "kill");
        }
      }
    }
  }
  ...
  function updateAlly(enemy, now, dt) {
    ...
    if (now >= enemy.allyUntil) { enemy.allyUntil = 0; enemy.state = "idle"; return false; }
    ...
    if (dist > CONFIG.enemyAttackRange) {
      ...
      damageEnemy(target, damage, 0, 0, now, "ally");
      enemy.allyNextAttack = now + 900;
      if (target && !target.isDead()) { target.staggerUntil = now + 220; target.state = "hurt"; }
    }
    return true;
  }
  ...
  window.updateEnemyCombat = function(enemy, now) {
    if (enemy && enemy.allyUntil) {
      if (updateAlly(enemy, now, 1 / 60)) return;
    }
  ...
  function drawAllyEffects() {
    ...
    for (const enemy of enemies) {
      ...
      const s = Camera.worldToScreen(enemy.x, enemy.y);
      ctx.save();
      ctx.strokeStyle = "rgba(70,170,255,.95)"; ctx.shadowColor = "#2d9cff"; ctx.shadowBlur = 12; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.arc(s.x, s.y - 20, 18, 0, Math.PI * 2); ctx.stroke();
      ctx.fillStyle = "#79c8ff"; ctx.font = "bold 10px Arial"; ctx.textAlign = "center"; ctx.fillText("ALIADO", s.x, s.y - 43);
      ctx.restore();
    }
  }
  ...
  function drawSummonerEffects() {
    ...
    for (const e of enemies) {
      ...
      if (e.type === "summonerBoss") {
        ctx.save();
        ctx.fillStyle = "#ef6d9b"; ctx.strokeStyle = "#ffd7e4"; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.ellipse(s.x, s.y - 27, 15 * CONFIG.zoom, 18 * CONFIG.zoom, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
        ctx.fillStyle = "#ffe9f1"; ctx.font = "bold 11px Arial"; ctx.textAlign = "center"; ctx.fillText("INVOCADOR", s.x, s.y - 53);
        if (e.summonFlashUntil && now < e.summonFlashUntil) {
          const p = 1 - (e.summonFlashUntil - now) / 900;
          ctx.strokeStyle = `rgba(255,140,190,${1 - p})`; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(s.x, s.y - 10, 18 + p * 55, 0, Math.PI * 2); ctx.stroke();
          for (let i = 0; i < 12; i++) {
            const a = i * Math.PI * 2 / 12 - p * 2, r = 12 + p * 50;
            ctx.fillStyle = `rgba(255,190,215,${1 - p})`; ctx.fillRect(s.x + Math.cos(a) * r - 2, s.y - 10 + Math.sin(a) * r - 2, 4, 4);
          }
        }
      }
      if (e.isSummonedMinion && now - e.summonBorn < 700) {
        const p = Math.min(1, (now - e.summonBorn) / 700);
        ctx.save(); ctx.strokeStyle = `rgba(255,190,215,${1 - p})`; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(s.x, s.y - 10, 8 + p * 22, 0, Math.PI * 2); ctx.stroke(); ctx.restore();
      }
    }
  }
